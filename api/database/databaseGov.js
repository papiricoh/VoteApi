...
const db = {
    ...
    async getGovernment() {
        ...
        try {
            const [rows] = await connection.query(`SELECT users.id AS user_id, government_members.id, government_members.role, government_members.depends_of, users.first_name, users.last_name, users.username, parties.label, parties.name, parties.color
